...
class CategoryViewState(StatesGroup):
    waiting_for_category = State()
...
class V2FilterState(StatesGroup):
    browsing         = State()  # список фільтрів
    waiting_for_name = State()
...
async def _v2_show_group_categories(message: Message, state: FSMContext, group_id: int):
    ...
    await state.update_data(v2_cats_group_id=group_id)
    await state.set_state(V2CategoryState.browsing)
    await message.answer(
        header,
        parse_mode="HTML",
        reply_markup=ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True),
...
@router.message(V2CategoryState.browsing)
async def v2_category_browsing_handler(message: Message, state: FSMContext):
    if not await require_admin(message):
        return
    text = (message.text or "").strip()
    data = await state.get_data()
    ...
    if text == "⬅️ Назад до групи":
        # Повернення до карточки групи
        if group_id:
            groups = await db.v2_list_product_groups()
            for g in groups:
                if g["id"] == group_id:
    ...
    if text == "➕ Додати категорію v2":
        await state.set_state(V2CategoryState.waiting_for_name)
        await message.answer(
...
@router.message(V2CategoryState.waiting_for_name)
async def v2_category_name(message: Message, state: FSMContext):
    if message.text == "⬅️ Назад":
        data = await state.get_data()
        gid = data.get("v2_cats_group_id")
        if gid:
            await _v2_show_group_categories(message, state, int(gid))
        else:
            await _v2_show_product_groups(message, state)
        return
    name = (message.text or "").strip()
    if not name:
        ...
        return
    await state.update_data(v2_cat_name=name)
    await state.set_state(V2CategoryState.waiting_for_emoji)
    await message.answer(
        f"📂 <b>Нова категорія</b> — крок 2/2\n\nНазва: <b>{name}</b>\n\n"
...
@router.message(V2CategoryState.waiting_for_emoji)
async def v2_category_emoji(message: Message, state: FSMContext):
    data = await state.get_data()
    name = data.get("v2_cat_name", "")
    group_id = data.get("v2_cats_group_id")
    if message.text == "⬅️ Назад":
        await state.set_state(V2CategoryState.waiting_for_name)
        await message.answer(
    ...
    slug = _make_category_key(name)
    if not slug or not group_id:
        await message.answer("⚠️ Помилка генерації slug або відсутній group_id. Спробуйте знову.")
        return
    try:
        await db.v2_create_category(
            group_id=int(group_id), name_uk=name, name_ru=name,
            emoji=emoji, slug=slug,
        )
    except Exception as _e:
        print(f"[v2_create_category] {_e}")
        await message.answer("⚠️ Помилка при збереженні.")
        return
    await message.answer(
        f"✅ Категорія <b>{emoji} {name}</b> створена!\nslug: <code>{slug}</code>",
        parse_mode="HTML",
    )
    await _v2_show_group_categories(message, state, int(group_id))
...
async def _v2_show_category_card(message: Message, state: FSMContext, cat: dict):
    ...
    await state.update_data(v2_viewing_cat_id=cat_id, v2_cats_group_id=group_id)
...
async def _v2_show_category_filters(message: Message, state: FSMContext, category_id: int):
    """Показує фільтри категорії кнопками."""
    fields = await db.v2_list_filter_fields_by_category(category_id)
    buttons = []
    buttons.append([KeyboardButton(text="➕ Додати фільтр")])
    for f in fields:
        label = (f["label_uk"] or f["label_ru"] or f["field_key"] or "?").strip()
        if label:
            buttons.append([KeyboardButton(text=label)])
    buttons.append([KeyboardButton(text="⬅️ Назад до категорії")])
    header = "🔧 <b>Фільтри категорії</b>" + (
        f"\n\nВсього: {len(fields)}" if fields else "\n\n<i>Поки немає жодного фільтра.</i>"
    )
    await state.update_data(v2_filters_cat_id=category_id)
    await state.set_state(V2FilterState.browsing)
    await message.answer(
        header,
        parse_mode="HTML",
        reply_markup=ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True),
...
@router.message(V2FilterState.browsing)
async def v2_filter_browsing_handler(message: Message, state: FSMContext):
    if not await require_admin(message):
        return
    text = (message.text or "").strip()
    data = await state.get_data()
    ...
    if text == "➕ Додати фільтр":
        await state.set_state(V2FilterState.waiting_for_name)
        await message.answer(
...
@router.message(V2FilterState.waiting_for_name)
async def v2_filter_name(message: Message, state: FSMContext):
    data = await state.get_data()
    category_id = data.get("v2_filters_cat_id")
    ...
    try:
        ...
        print(f"[v2_create_filter_field] {_e}")
        await message.answer("⚠️ Помилка при збереженні.")
        return
...
async def _v2_show_category_brands(message: Message, state: FSMContext, category_id: int):
    """Показує бренди категорії через ReplyKeyboard."""
...
async def category_view_category_selected(message: Message, state: FSMContext):
    if message.text == "⬅️ Назад":
        await state.clear()
        await message.answer("Справочники:", reply_markup=directories_kb)
        return
...
async def category_view_action(message: Message, state: FSMContext):
    data = await state.get_data()
    cat_key_val = data.get("cat_key", "")
    cat_name_val = data.get("cat_name", "")
    ...
    if message.text == "➕ Добавить фильтр":
        await state.set_state(CategoryViewState.waiting_for_filter_name)
        await message.answer(
            f"📂 Категорія: <b>{cat_name_val}</b>\n\n"
            "🔤 Введіть назву фільтра, яку побачить покупець на сайті:\n"
...
async def category_view_filter_name(message: Message, state: FSMContext):
    if message.text == "⬅️ Назад":
        data = await state.get_data()
        await _show_category_card(message, state, data.get("cat_key", ""))
        return
    ...
    if not label:
        ...
        return
    data = await state.get_data()
    cat_key_val = data["cat_key"]
    field_key = _resolve_field_key(label)
    await db.create_filter_field(
        category_key=cat_key_val,
        field_key=field_key,
        label_ru=label,
        label_uk=label,
        field_type="select",
    )
    await message.answer(f"✅ Фільтр «<b>{label}</b>» додано.", parse_mode="HTML")
    await _show_category_card(message, state, cat_key_val)
...
async def site_home(request: Request, q: str = "", category: str = "", page: int = 1, brand: str = "", price_min: str = "", price_max: str = "", in_stock: str = "", volume: str = "", sort: str = ""):
    ...
    _AUTO_NEW_FILTERS_CATS = {"boilers", "air_conditioners", "refrigerators"}
    ...
    if not _new_active and target_key_dyn in ("boilers", "air_conditioners", "refrigerators", "washing_machines", "hoods", "microwaves", "gas_stoves"):
        try:
    ...
    products_for_options = list(products) if (dyn_attrs and not _new_active) else []

    if dyn_attrs and not _new_active:
        qp = request.query_params
